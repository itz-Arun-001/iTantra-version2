...
### Web UI (currently still v1's pipeline — English/Whisper-only STT accuracy applies)
...
Then open **http://localhost:3000**, pick a language and bitrate mode, and hit record. Note: as of this version, the web UI has not yet been updated to use IndicConformer or the real UDP transport — see [Roadmap](#-roadmap).
...
- 🔁 Wiring the real UDP transport and IndicConformer STT into the polished web UI, so the demo people see matches the real, validated pipeline
...
- [ ] **Wire IndicConformer and the real UDP transport into `api_server.py`** so the polished web demo reflects the validated v2 pipeline instead of the original v1 (Whisper-only, simulated-transport) one
...
- **The web UI (`api_server.py` + `itantra-ui/`) has not yet been updated to v2** — it still runs the original Whisper-only pipeline with simulated (not real) transport. The dual-STT branching and real UDP transport currently only exist in the CLI scripts (`network_sender.py`/`network_receiver.py`)
